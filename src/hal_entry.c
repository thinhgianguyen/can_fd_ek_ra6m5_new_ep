...
#define SW1 ((bsp_io_port_pin_t) BSP_IO_PORT_00_PIN_05)
#define SW2 ((bsp_io_port_pin_t) BSP_IO_PORT_00_PIN_04)

#define LED1 ((bsp_io_port_pin_t) BSP_IO_PORT_00_PIN_06)
...
void hal_entry(void)
{
    ...
    /*Variables are used to check status of switches*/
    uint8_t common_status = 0;
    uint8_t new_common_status = 0;
    while(1)
    {
...
//            canfd_operation();
        ...
        uint32_t sw2_status = R_BSP_PinRead(SW2);
        uint32_t sw1_status = R_BSP_PinRead(SW1);

        /*Ensure the write in CAN is process one time*/
        if (sw1_status == 0)
        {
            new_common_status = 1;
        }
        ...
        /*Write function if user click different switch*/
        if (common_status != new_common_status)
        {
            common_status = new_common_status;
            if (common_status == 1)
            {
                R_BSP_PinWrite(LED1, BSP_IO_LEVEL_HIGH);
                sw1_operation();
            }
            else if (common_status == 2)
            {
                R_BSP_PinWrite(LED1, BSP_IO_LEVEL_HIGH);
                sw2_operation();
            }
        ...
        if (g_freq>0)
        {
            blink_led(g_freq, (bsp_io_port_pin_t) LED2);
        }
        else
        {
            R_BSP_PinWrite((bsp_io_port_pin_t) LED2, BSP_IO_LEVEL_LOW);
        }
...
/*******************************************************************************************************************//**
 * @brief       This function is to blink LED
 * @param[in]   None
 * @return      None
 **********************************************************************************************************************/
void blink_led(uint8_t freq, bsp_io_port_pin_t pin)
{
    /* Set the blink frequency (must be <= bsp_delay_units */
    uint32_t freq_in_hz = freq;
